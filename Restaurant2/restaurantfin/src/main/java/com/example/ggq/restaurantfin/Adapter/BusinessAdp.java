...
public class BusinessAdp extends RecyclerView.Adapter<BusinessAdp.ViewHolder> {

    //用来记录是否为head
    public static final int TYPE_HEADER = 0;
    public static final int TYPE_NORMAL = 1;
    private ArrayList<Business> mdate;
    ...
    //构造函数
    public BusinessAdp(Context context, ArrayList<Business> muser) {
    ...
    public void addDatas(ArrayList<Business> datas) {
        mdate.addAll(datas);
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        final Business user = mdate.get(pos);
        Glide.with(context)
                .load(baseUrl + user.getComboPhoto() + ".jpg")
                .diskCacheStrategy(DiskCacheStrategy.ALL)
                .error(R.drawable.image_not_upload)
                .into(holder.binding.photo);
        holder.binding.date.setText("订单日期：" + user.getBusinessDate());
        holder.binding.Name.setText(user.getComboName());
        holder.binding.setBusiness(user);
